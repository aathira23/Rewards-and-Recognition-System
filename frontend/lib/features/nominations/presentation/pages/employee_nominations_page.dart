                          ...
                          child: Container(
                            ...
                            child: BlocBuilder<RecognitionsBloc, RecognitionsState>(
                              builder: (context, state) {
                                final awardsFeed = state.feed.where((r) => (r.sourceType ?? '').toUpperCase() == 'AWARD').toList();
                                if (state.status == RecognitionStatus.loading && state.feed.isEmpty) {
                                  return const Center(child: CircularProgressIndicator());
                                }
                                if (awardsFeed.isEmpty) {
                                  return Center(child: Text('No recent awards', style: AppTextStyles.body(color: Colors.grey[500])));
                                }

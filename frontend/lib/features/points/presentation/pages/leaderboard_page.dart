...
class _LeaderboardPageState extends State<LeaderboardPage> {
              ...
              child: SingleChildScrollView(
                ...
                child: BlocBuilder<PointsBloc, PointsState>(
                  builder: (context, state) {
                    if (state.status == PointsStatus.loading && state.leaderboard.isEmpty) {
                      return const Center(child: CircularProgressIndicator());
                    }

                    if (state.leaderboard.isEmpty && state.status == PointsStatus.success) {
                      return const Center(
                        child: Padding(
        ...
        builder: (context, constraints) {
          ...
          if (isMobile) {
             ...
             return Row(
                ...
                crossAxisAlignment: CrossAxisAlignment.end,
                children: [
                  if (rank2 != null) Expanded(child: _buildPodiumItem(rank2, 2, 140, const Color(0xFFF3F4F6))),
                  if (rank1 != null) Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 8), child: _buildPodiumItem(rank1, 1, 180, const Color(0xFFFEF9C3)))),
                  if (rank3 != null) Expanded(child: _buildPodiumItem(rank3, 3, 120, const Color(0xFFFFF7ED))),
                ],
              );
          }
          ...
          return Row(
            ...
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              if (rank2 != null) _buildPodiumItem(rank2, 2, 140, const Color(0xFFF3F4F6), width: 140),
              if (rank1 != null) Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: _buildPodiumItem(rank1, 1, 180, const Color(0xFFFFFBEB), width: 160)),
              if (rank3 != null) _buildPodiumItem(rank3, 3, 120, const Color(0xFFFFF7ED), width: 140),
            ],
          );
        },
      )
  ...
  Widget _buildPodiumItem(LeaderboardEntryEntity entry, int rank, double height, Color boxColor, {double? width}) {
    Color medalColor;
    IconData iconData = Icons.emoji_events;
    if (rank == 1) medalColor = const Color(0xFFEAB308); // Gold
    else if (rank == 2) medalColor = const Color(0xFF9CA3AF); // Silver
    else medalColor = const Color(0xFFD97706); // Bronze
    ...
    return Column(
      ...
      children: [
        ...
        Text(entry.name, style: AppTextStyles.bodyBold(color: Colors.black87), maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center),
        const SizedBox(height: 4),
        Row(
          mainAxisSize: MainAxisSize.min,
          children: [
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text('${entries.length} Participants', style: AppTextStyles.body(color: Colors.grey[600])),
            ],
          ),
          const SizedBox(height: 24),
          ListView.separated(
  ...
  Widget _buildListRow(LeaderboardEntryEntity entry) {
    Widget rankWidget;
    if (entry.rank == 1) {
      rankWidget = const Icon(Icons.emoji_events, color: Color(0xFFEAB308), size: 24);
    } else if (entry.rank == 2) {
      rankWidget = const Icon(Icons.military_tech, color: Color(0xFF9CA3AF), size: 24);
    } else if (entry.rank == 3) {
      rankWidget = const Icon(Icons.military_tech, color: Color(0xFFD97706), size: 24);
    } else {
      rankWidget = SizedBox(
        width: 24,
        child: Center(
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(entry.name, style: AppTextStyles.bodyBold(color: Colors.black87)),
              if (entry.departmentName != null && entry.departmentName!.isNotEmpty)
                Text(entry.departmentName!, style: AppTextStyles.small(color: Colors.grey.shade500)),
            ],
          ),
        ),
        Column(
